...
class Orders
{
    ...
    public function addOrder()
    {
        $connection = openDB();
        if($connection){
            $insertQuery = "INSERT INTO orders SET Notes = '$this->Notes' , Date = '$this->Date', Status = 0 , Room = $this->Room, Uid = $this->Uid, TotalPrice = $this->TotalPrice";
            $res = mysqli_query($connection, $insertQuery);
            if($res)
            {
                $orderDetails = "INSERT INTO product_order SET Oid = (SELECT Oid FROM orders ORDER BY Oid LIMIT 1) , Pid = '$this->id', Amount = $this->Quantity";
                $resDetails = mysqli_query($connection, $orderDetails);
